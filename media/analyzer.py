...
class MediaAnalyzer:
    ...
    async def _analyze_images_concurrent(self, image_urls: List[str]) -> List[Dict]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            image_data_results = await asyncio.gather(*image_data_tasks, return_exceptions=True)
            
            # Filter out failed downloads and prepare for analysis
            valid_images = []
            ...
            # Analyze images concurrently with rate limiting
            analysis_tasks = [
                self._analyze_single_image(url, image_data)
